  ...
  try {
    const [voteResult, questionResult] = await Promise.all([
      supabase
        ...
        .select('id, full_name, anonymous, selected_topics, created_at')
    ...
    if (voteResult.error) {
      console.error('event-e2-admin-list vote query failed:', voteResult.error);
      return res.status(500).json({ error: mapQueryError(voteResult.error) });
    }
    if (questionResult.error) {
      console.error('event-e2-admin-list question query failed:', questionResult.error);
    ...
    const voteItems = (voteResult.data || []).map((row) => ({
      ...
      selected_topics: normalizeSelectedTopics(row.selected_topics),
    ...
    console.error('event-e2-admin-list failed:', error);
